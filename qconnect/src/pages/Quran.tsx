...
const Quran: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col font-body relative overflow-x-hidden selection:bg-[#00695C]/20 text-neutral-800">
      ...
      {/* --- FLUID AMBIENT BACKGROUND GLOWS --- */}
      ...
      {/* --- STICKY NAVBAR --- */}
      <nav className="sticky top-0 z-[100] w-full bg-white/95 backdrop-blur-2xl border-b border-neutral-100 shadow-md safe-top transition-shadow duration-300">
        <div className="max-w-6xl mx-auto px-4 md:px-8 py-3 flex items-center gap-3">
          ...
          <div className="flex-grow relative">
            ...
            <input
              ...
              className="w-full bg-neutral-50/50 border border-neutral-100 py-2.5 pl-9 pr-4 rounded-xl focus:bg-white focus:border-[#00695C]/30 focus:outline-none text-sm font-medium text-neutral-800 placeholder:text-neutral-300 transition-all shadow-inner"
            />
          </div>
        </div>
      </nav>
      ...
      <main className="flex-grow w-full relative z-10 max-w-6xl mx-auto px-4 md:px-8 pt-6 md:pt-10 pb-32">
        <h1 className="text-xl md:text-2xl font-display font-black tracking-tight text-neutral-800 mb-5 md:mb-8 flex items-center gap-2">
           The Holy <span className="text-[#00695C]">Quran</span>
           <span className="w-1.5 h-1.5 rounded-full bg-[#AEEA00] animate-pulse"></span>
        </h1>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          {filteredSurahs.map((s, idx) => {
             const delay = Math.min(idx * 0.04, 1.5);
             return (
               <div 
                 key={s.id}
               ...
               >
                   ...
                   <div className="absolute -right-6 -bottom-12 text-[140px] font-arabic leading-none text-[#00695C] opacity-[0.02] select-none pointer-events-none group-hover:scale-110 group-hover:opacity-[0.04] group-hover:-translate-y-4 group-hover:-rotate-3 transition-all duration-[1500ms]" >
                      {s.name_arabic}
                   </div>
                   <div className="relative z-10 p-6 flex flex-col h-full justify-between">
                       <div className="flex items-start justify-between">
                           <div className="w-12 h-12 rounded-[18px] bg-white flex items-center justify-center border border-neutral-100 text-[#00695C] font-display font-black text-xl shadow-sm group-hover:bg-[#00695C] group-hover:text-white group-hover:border-[#00695C] transition-all duration-500">
                               {s.id}
                           </div>
                           <div className="flex flex-col items-end gap-1.5">
                                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/80 border border-white shadow-sm group-hover:bg-[#00695C]/5 transition-colors">
                                   ...
                                   <span className="text-[9px] uppercase font-black tracking-widest text-neutral-600">{s.revelation_place}</span>
                                </div>
                                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/80 border border-white shadow-sm group-hover:bg-[#00695C]/5 transition-colors">
                                   <Layers size={10} className="text-[#AEEA00]" />
                                   <span className="text-[9px] uppercase font-black tracking-widest text-neutral-600">{s.verses_count} Verses</span>
                                </div>
                           </div>
                       </div>
      ...
      <button 
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 z-[110] w-14 h-14 bg-[#00695C] text-white rounded-2xl shadow-2xl flex items-center justify-center transition-all duration-500 active:scale-90 ${showBackToTop ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0'}`}
